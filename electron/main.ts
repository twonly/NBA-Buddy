...
import { listPacks, ensureCharactersDir, charactersDir, packById, DEFAULT_PACK_ID } from './characters';
...
function loadConfig(): Config {
  ...
  try {
    ...
    cfg = {
      favoriteTeams: ['San Antonio Spurs', 'New York Knicks'],
      mode: 'live',
  ...
  if (filtered.length === 0) {
    cfg.favoriteTeams = ['San Antonio Spurs', 'New York Knicks'];
  } else {
...
function rebuildTrayMenu() {
  ...
  const menu = Menu.buildFromTemplate([
    { label: `🦐 篮球虾仔  ·  ${cfg.mode === 'replay' ? 'Replay 模式' : 'Live 模式'}`, enabled: false },
    { type: 'separator' },
...
async function saveDailyCard(): Promise<string | null> {
  ...
  const cfg = loadConfig();
  const my = cfg.favoriteTeams?.[0] ?? 'Spurs';
  ...
  type CardPayload = {
    ...
    status?: string; period?: number; clock?: string;
  ...
  if (live) {
    ...
    const draw = live.myScore === live.oppScore;
    const tag = live.status === 'STATUS_FINAL' || live.status === 'STATUS_END_OF_GAME' ? '终场' :
                live.status === 'STATUS_IN_PROGRESS' ? `Q${live.period} ${live.clock}` :
                live.status === 'STATUS_HALFTIME' ? '半场' :
                live.status === 'STATUS_END_PERIOD' ? `Q${live.period} 末` :
                live.status === 'STATUS_SCHEDULED' ? '未开赛' : '';
    payload = {
      ...
      status: live.status, period: live.period, clock: live.clock,
  ...
  const filename = `篮球虾仔-战报-${new Date().toISOString().slice(0, 10)}-${Date.now() % 100000}.png`;
