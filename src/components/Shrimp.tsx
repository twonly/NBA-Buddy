...
function statusBadge(s: ScoreState, now: number): string {
  if (s.status === 'STATUS_IN_PROGRESS') return `Q${s.period} ${s.clock}`;
  if (s.status === 'STATUS_HALFTIME') return '半场';
  if (s.status === 'STATUS_END_PERIOD') return `Q${s.period} 末`;
  if (s.status === 'STATUS_FINAL' || s.status === 'STATUS_END_OF_GAME') return '终';
...
export function Shrimp() {
  ...
  const seriesLabel = (() => {
    if (!score) return '';
    const parts: string[] = [];
    if (score.gameNumber) parts.push(`G${score.gameNumber}`);
    if (score.seriesMyWins !== undefined && score.seriesOppWins !== undefined) {
      parts.push(`${score.seriesMyWins}-${score.seriesOppWins}`);
    } else if (score.seriesSummary) {
      parts.push(score.seriesSummary);
    }
    return parts.join(' · ');
  })();

  const isPregame = score?.status === 'STATUS_SCHEDULED';
    ...
    >
      {score && (
        <div className="score-stack">
          ...
          {seriesLabel && (
            <div className="series-chip">{seriesLabel}{score.seriesBestOf ? ` · Bo${score.seriesBestOf}` : ''}</div>
          )}
          {showWinProb && winProb.length > 1 && score.status === 'STATUS_IN_PROGRESS' && (
